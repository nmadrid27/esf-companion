...
Read the Active Contexts section of the companion state file (`projects/_esf/companion-state.md`) for any course-specific Position Statement or ESF requirements. If the user's course specifies additional Position Statement elements (e.g., Design Intent, AI use planning), include those in the gate check. If no course context exists, use the default three-element Position Statement (stance, what matters most, what you will not compromise on).
...
**HARD STOP: The user must confirm before exploration begins.** After presenting the readability pass, stop. Do not provide tensions, alternatives, challenges, or any exploration content in the same response. Wait for the user's explicit confirmation that the cleaned version still says what they meant. If they flag anything that changed meaning, revise and wait again. Only after the user confirms does exploration begin. The confirmed version becomes the working Position Statement for the rest of the project.
...
|---|---|---|
| Position Statement gate clears (Phase 2 to 3) | PS path, date, project name, confirmation status | Update companion state file: Current Project section |
...
| Phase transition | New phase, what was completed | Update companion state file: Current Project phase field |
...
5. Clear the session buffer: use the Write tool to overwrite `projects/[context]/logs/.session-buffer.md` with empty content. Then read the file back to confirm it is empty. If stale content remains, delete the file and recreate it empty. Do not report session end until the buffer is verified clear.
6. Update `projects/_esf/companion-state.md` Current Project section with the current phase and last activity date
...
**Where to store:** Append to `projects/_esf/companion-state.md` under the "Growth Record" section. Each completed project adds one entry. Over the full course sequence (DISCOVER through DESIGN), this builds a visible development arc.
...
Read the Active Contexts section of `projects/_esf/companion-state.md` for the user's scaffolding level (Guided, Supported, or Independent). Calibrate tone and gate strictness accordingly:
