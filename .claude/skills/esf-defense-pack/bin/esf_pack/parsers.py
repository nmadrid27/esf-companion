...
# Backtick (```...```) and tilde (~~~...~~~) fenced code blocks. Both are
# CommonMark fenced code per the spec; tilde fences are used by students whose
# templates were generated by tooling like Pandoc or Hugo. Without tilde
# support, an `## heading` inside a `~~~` block falsely terminates the
# surrounding section.
_FENCED_CODE_RE = re.compile(r"```.*?```|~~~.*?~~~", re.DOTALL)
# Indented code blocks (CommonMark §4.4): a run of lines each beginning with
# four spaces or a tab. Preceded by a blank line (or document start). An H2
# inside an indented block would already be missed by `_H2_RE` because the
# leading whitespace shifts `##` off column 0, but masking these matches as
# well keeps the strip-then-parse story consistent across all three fence
# kinds and protects downstream consumers that may key off `_FENCED_CODE_RE`
# alone in future.
_INDENTED_CODE_BLOCK_RE = re.compile(
    r"(?:^|\n)\n((?:(?:    |\t)[^\n]*(?:\n|$))+)",
)
...
def _mask_code_blocks(body: str) -> str:
    """Return `body` with fenced and indented code-block content blanked out.

    The masking preserves length and newline positions (each masked character
    becomes a space, except for newlines, which stay), so all subsequent regex
    offsets line up with the original body. We mask all three CommonMark code
    forms — backtick-fenced, tilde-fenced, and 4-space/tab indented — so an
    H2-looking line embedded in any of them cannot falsely terminate a section.
    """
    def _blank_preserving_newlines(match: re.Match) -> str:
        s = match.group(0)
        # Preserve newlines so MULTILINE/line-offset semantics stay intact.
        return "".join(ch if ch == "\n" else " " for ch in s)

    masked = _FENCED_CODE_RE.sub(_blank_preserving_newlines, body)
    masked = _INDENTED_CODE_BLOCK_RE.sub(_blank_preserving_newlines, masked)
    return masked
...
def extract_sections(body: str) -> dict:
    ...
    Strips fenced and indented code blocks before splitting so that an H2-looking
    line *inside* a code block doesn't falsely terminate the surrounding section.
    Code-block content within a section is preserved by replacing matched code
    with same-length whitespace before regex matching, then restoring; in our
    markdown-template use case ESF templates don't currently use code blocks,
    but this guard prevents the worst-case parser confusion silently dropping
    section content if the convention ever changes.
    """
    placeholder_body = _mask_code_blocks(body)
    sections = {}
...
# Heading-normalization helpers for the element-alias matcher: strip the
# trailing parenthetical, and capture its contents separately.
...
# Markdown heading line of any depth (#, ##, ###, ...). Used as a block
# boundary when walking the text around an inline @-tag.
_MARKDOWN_HEADING_RE = re.compile(r"^#{1,6}\s")
# Placeholder fill-in detector used to skip template scaffolding blocks like
# `[what you ...]` / `[brief ...]` / `[tag ...]` that students leave un-edited.
_PLACEHOLDER_RE = re.compile(r"\[(what|brief|tag).*?\]", re.IGNORECASE)
# Used inside parse_reflection to terminate Kept/Revised/Rejected blocks at the
# start of the NEXT labeled block, not at any inline `**bold**` token. A naive
# `(?=\n\s*\*\*|$)` truncates the field as soon as the student writes an
# emphasized phrase, silently dropping content from the bold onward.
# Stored as a raw string (not compiled) because it's substituted into a larger
# pattern with a dynamic label rather than used directly.
_NEXT_KRR_LABEL_RE = (
...
def _extract_paragraph_around(text: str, pos: int) -> str:
    ...
    while cursor > 0:
        ...
        if not prev_line.strip():
            break
        if _MARKDOWN_HEADING_RE.match(prev_line) or prev_line.strip() == "---":
    ...
    while end_cursor < len(text):
        ...
        if not next_line.strip():
            break
        if _MARKDOWN_HEADING_RE.match(next_line) or next_line.strip() == "---":
            break
