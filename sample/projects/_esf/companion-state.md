# ESF Companion State

This file holds mutable local state for the Claude Code workflow.

## Identity

- **Name:** Alex Rivera
- **Preferred name:** Alex
- **Role or program:** BFA student in Motion Media Design
- **Discipline or focus:** Creative computing, responsive systems, and audiovisual interaction
- **Current period:** Spring 2026

## Active Contexts

- build-course: BUILD level course
  Collaborator or lead: Professor Chen
  ESF level: full
  Records of Resistance required: yes, minimum 3
  Position Statement timing: project start

## Current Project

- **Context:** build-course
- **Project name:** responsive-system
- **Brief location:** `projects/build-course/briefs/p2-responsive-system.md`
- **Position Statement:** `projects/build-course/position-statements/responsive-system.md`
- **Phase:** Explore
- **Last session:** none yet
- **Scaffolding level:** Independent

## Preferences

- **silent_mode:** false

## Growth Record

None yet.
