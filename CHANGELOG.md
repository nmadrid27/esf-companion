...
### Added
- **Landing page (local, index.html).** Reviewed Figma Make prototype and built improved version: before/after scenario cards in the hero; "What is AI drift?" section preceding the process; amber gate callout mid-process ("AI cannot enter until your Position Statement exists — this is not a rule, it is the mechanism that keeps your work yours"); Five Questions moved below the process flow as numbered cards; Install section restructured with one dominant CTA card (curl + Claude Code) and three secondary cards; FAQ accordion with five blocking questions. File kept local and added to `.gitignore` — not in public repo.
...
- **Moment 1 Nudge mode.** Splits Moment 1 into nudge mode (default) and gate mode. Nudge mode prepends a one-line hint (`[ESF: no Position Statement for [doc] — note one?]`) without pausing. Fires on first Write/Edit per document per session; re-fires on structural edits (claim assertion, first-person observation-as-evidence, attributed quote, specific datum, argument/frame change). Formatting, phrasing, typo/citation tidying, wikilink repair, and frontmatter corrections do not trigger. Max two nudges per document per session (in-context count, resets at session start); first decline silences first-touch, a structural edit re-fires once, second decline silences for the session. Gate mode preserved for bulk production, new-project initiation, brief-frontmatter requirement, and context-level requirement. Motivation: the previous gate-only design never fired during incremental editing work on existing documents, leaving Position Statement gaps unsurfaced across multi-turn sessions. Spec updated in `.claude/agents/esf-companion.md`, `install.sh` ambient heredoc, and `platforms/cowork/skills/esf-project/SKILL.md`.
