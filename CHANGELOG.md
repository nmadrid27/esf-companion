...
- **Moment 1 Nudge mode.** When producing substantive content and no Position Statement exists for the work, the Companion prepends a one-line nudge (`[ESF: no Position Statement for [doc] — note one?]`) rather than pausing. Fires on first Write/Edit per document per session, and re-fires on structural edits (claim assertion, first-person observation-as-evidence, attributed quote, specific datum, argument/frame change). Formatting, phrasing, typo/citation tidying, wikilink repair, and frontmatter corrections do not trigger. Max two nudges per document per session; first decline silences first-touch, a structural edit re-fires once, second decline silences for the session. Gate mode preserved for bulk production and new-project initiation. Motivation: Moment 1's "before producing substantive content on a new project" trigger never fired during incremental drafting/editing work on existing documents, leaving Position Statement gaps unsurfaced across multi-turn sessions. Spec updated in `.claude/agents/esf-companion.md`, `install.sh` ambient heredoc, and `platforms/cowork/skills/esf-project/SKILL.md`.
